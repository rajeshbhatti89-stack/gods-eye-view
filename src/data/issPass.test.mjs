...
const DELHI = { latDeg: 30.2672, lonDeg: -97.7431 };
...
test('finds a structurally-consistent ISS pass within 24h of the TLE epoch', () => {
  ...
  const pass = findNextIssPass({ satrec, ...DELHI, fromMs: FROM_MS, minElevDeg: 10 });
  ...
  const peak = lookAnglesAt(satrec, pass.maxElevMs, DELHI.latDeg, DELHI.lonDeg);
  assert.ok(peak && Math.abs(peak.elevDeg - pass.maxElevDeg) < 1.5);
  const before = lookAnglesAt(satrec, pass.riseMs - 60_000, DELHI.latDeg, DELHI.lonDeg);
  assert.ok(before && before.elevDeg < 10);
});

test('returns null when no pass clears an absurd threshold', () => {
  const satrec = twoline2satrec(L1, L2);
  const pass = findNextIssPass({ satrec, ...DELHI, fromMs: FROM_MS, minElevDeg: 89.9, horizonHours: 2 });
